...
def getBVecs(bvalFileName, bvecFileName, nifti):
    ...
    length=image.get_header().get_data_shape()[3]
    bvecs=numpy.zeros(length*3)
    bvals=numpy.zeros(length)

    bvals = []
    with open(bvalFileName) as f:
        bvals = [float(i) for line in f.readlines() for i in line.split()]
...
if __name__ == '__main__':
    ...
    for nifti in args.files:
        [path, afile] =os.path.split(nifti)
        #bvalFileName = os.path.join(path, 'bval')
	bvalFileName = 'C20.bval'

        [path, afile] = os.path.split(nifti)
        #bvecFileName = os.path.join(path, 'bvec')
	bvecFileName = 'C20.bvec'        
